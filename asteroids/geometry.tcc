...
bool Sphere<FLOAT, N>::intersects(Sphere<FLOAT, N> sphere) const {
  return ((Vector<FLOAT,N>)(center-sphere.center)).length()  <= radius + sphere.radius;
}

template <class FLOAT, size_t N>
bool  Sphere<FLOAT, N>::inside(Vector<FLOAT, N> point) const {
  return  ((Vector<FLOAT,N>)(center-point)).length() <= radius;
}
// solution via
